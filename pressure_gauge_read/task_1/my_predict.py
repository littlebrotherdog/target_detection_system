...
# 加载模型
model = YOLO('runs/pose/train3/weights/best.pt')  # 预训练的 YOLOv8n 模型
objs_labels = model.names
# 在图片列表上运行批量推理
results = model(['../DATASETS/images/test/1.jpg'], save=False, imgsz=640)  # 返回 Results 对象列表

frame = cv2.imread("../DATASETS/images/test/1.jpg")
